var SdenvExtend = (function () {
  ...
  function getRuntimeData(name, key) {
    return get(this, ['cache.runtime', name, key]
  ...
  const setFuncNative = function(func, name, len) {
  ...
  const setToString = function(target) {
    // 修改方法的name属性值
  ...
  const setNativeFuncName = function(func, name, len) {
  ...
  const _setFuncInit = function(win) {
    setFuncNative(toString, 'toString');
    win.Function.prototype.toString = toString;
    toString.__proto__ = win.Function.prototype;
  };

  const wrapFunc = function(obj, name, callback, isBind = true) {
  ...
  function delay(ms) {
    const self = this;
    return new Promise(resolve => get(self, 'memory.setTimeout', setTimeout)(resolve, ms));
  }
    ...
    win.eval = this.getTools('setNativeFuncName')(new Proxy(win.eval, {
      apply(target, thisArg, params) {
        ...
        const new_params = params.map((param) => {
          if (evalmap[param]) return evalmap[param];
  ...
  */
  ...
  function requestHandle(config) {
    ...
    if (typeof config !== 'object') config = {};
    const win = this.memory.window;
    self._registerConfig('handle:request', config, () => {
      config.log = (l => {
        if (!l || typeof l === 'function') return l;
        return (type, ...p) => {
          if (Array.isArray(l) && !l.includes(p[0])) return;
          self.memory.window.console.log(`【REQUEST HANDLE】${type}: `, p[0]);
        }
      })(config.log);
    });
    win.XMLHttpRequest = new win.Proxy(win.XMLHttpRequest, {
      construct(target, args, newTarget) {
        const xhr = win.Reflect.construct(target, args, newTarget);
        xhr.__xhr = { headers: {} };
        return xhr;
      }
    });
    win.XMLHttpRequest.prototype.open = function (method, url, async) {
      Object.assign(this.__xhr, { method, url, async });
      config.log?.('open', this.__xhr);
      config.cb?.('open', this.__xhr);
      return self.memory['XMLHttpRequest.prototype.open'].apply(this, arguments);
    };
    win.XMLHttpRequest.prototype.setRequestHeader = function (key, value) {
      this.__xhr.headers[key] = value;
      config.log?.('header', this.__xhr);
      config.cb?.('header', this.__xhr);
      return self.memory['XMLHttpRequest.prototype.setRequestHeader'].apply(this, arguments);
    };
    win.XMLHttpRequest.prototype.send = function (body) {
      this.__xhr.body = body;
      config.log?.('send', this.__xhr);
      config.cb?.('send', this.__xhr);
      const xhr = this;
      function onLoad() {
        try {
          Object.assign(xhr.__xhr, {
            status: xhr.status,
            responseURL: xhr.responseURL,
            responseType: xhr.responseType,
            response: xhr.responseType && xhr.responseType !== 'text'
              ? xhr.response
              : xhr.responseText
          });
          config.log?.('load', xhr.__xhr);
          config.cb?.('load', xhr.__xhr);
        } catch (e) {}
      }
      function onError() {
        config.log?.('error', xhr.__xhr);
        config.cb?.('error', xhr.__xhr);
      }
      this.addEventListener('load', onLoad);
      this.addEventListener('error', onError);
      return self.memory['XMLHttpRequest.prototype.send'].apply(this, arguments);
    };
  }
...
SdenvExtend.version='V1.8.2';SdenvExtend.buildTime='2025-12-22T05:02:56.841Z'
